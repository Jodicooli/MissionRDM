...
describe('Game Logic Level 2', () => {
  ...
  it('handles hint code (7) in English locale', () => {
  ...
  it('handles hint code (6) in French locale', () => {
  ...
  it('rejects wrong hint code for locale', () => {
    store.currentScenarioImage = 'final'
    logic.handleCodeInput('6') // Should be '7' for English
    expect(setFeedback).toHaveBeenCalledWith('error', '')
    expect(store.firstHintFound).not.toBe(true)
  })

  it('handles continue code (11) when hint is found', () => {
    store.currentScenarioImage = 'final'
